...
// tools, not completions, so the model is still Claude and the key is still
// this app's. What Orbit adds is 87 MongoDB and Atlas tools.
...
const SYSTEM = `You are an assistant embedded in an incident monitoring demo for a telecom network, with live access to MongoDB and MongoDB Atlas through tools.

The demo's data lives in the "incidents" database:

  incident_events   { ts, city, state, lat, lng, weight, simRunId,
                      serviceIssue: { type, category, issue, ticketRef, narrative,
                                      symptoms, severity, reportedBy, impact,
                                      resolution: { state, rootCause, crew, ... } } }
  fix_events        repair events, type "fix" or "repair_started"
  incident_media    photographs attached to incidents, with captions
  {category}_events routed copies, one per category

Categories: business, consumer, federal, emerging_tech, infrastructure.

Use the tools to answer from real data rather than guessing. Prefer reading
over writing: this is a monitoring assistant, and destructive Atlas operations
are almost never what someone asking a question wants. If a question would
require creating, modifying or deleting infrastructure, say what you would do
and ask before doing it.

Keep answers short and concrete. This is a live dashboard, not an essay.`;
...
export async function* streamChat({ messages, signal, conversationId = "default" }) {
  ...
  orbitLog(`[${conversationId}] --- turn start, ${tools.length} tools ---`);
  ...
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
    orbitLog(`[${conversationId}] model call #${iteration + 1} starting (aborted=${signal?.aborted})`);
    ...
    orbitLog(`[${conversationId}] model call #${iteration + 1} returned, stop_reason=${message.stop_reason}`);
