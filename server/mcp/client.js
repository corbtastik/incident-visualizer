...
function installErrorHandler(transport, key) {
  transport.onerror = (err) => {
    ...
    if (SSE_404.test(message)) {
      orbitLog(`[${key}] SSE GET 404 ignored (server is POST-only)`);
      return;
    }
    orbitLog(`[${key}] transport error, connection KEPT: ${message}`);
  };
}
...
export async function getMcpClient(conversationId = "default") {
  ...
  const client = new Client({ name: "incident-visualizer", version: "0.1.0" });
  ...
  orbitLog(`[${conversationId}] MCP session ${transport.sessionId ?? "(none)"} ESTABLISHED`);
  return session;
}
